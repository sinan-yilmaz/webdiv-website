...
/* 320 vh Strecke, Inhalt sticky: Woerter fuellen sich wortweise (Scroll-Fenster
   p 0.06–0.66), die Kobalt-Linie zeichnet sich p 0.5–0.92, vier Objekte pendeln
   in flachem 3D (24 s Zyklus, ±6° Zeiger-Reaktion, leichtes Floaten). */
function StatementSection() {
  ...
  useFrame(({ now, smoothY, vh, mouse }) => {
    ...
    const st = stateRef.current;
    const p = clamp((smoothY - st.top) / (st.height - vh), 0, 1);
    ...
    <section className="statement" data-theme="dark" aria-label={statement.ariaLabel} ref={sectionRef}>
      <div className="statement-inner bg-dots-dark" data-rails>
        ...
        <p className="statement-text display2" ref={textRef}>
          <strong className="w-strong">{statement.lead}</strong>
